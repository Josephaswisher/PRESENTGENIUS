        ...
        <div className="flex-1 overflow-y-auto bg-zinc-950 p-8">
          ...
          {mode === 'code' && (
            <textarea
              ...
              onChange={(e) => {
                ...
                setSlides(parseHtmlToSlides(e.target.value));
              }}
          ...
          {mode === 'preview' && (
            <div className="w-full h-full bg-zinc-900 rounded-xl overflow-hidden border border-zinc-800">
              <iframe
                srcDoc={rawHtml} // Always show rawHtml which is updated on mode switch
...
// Helper: Parse HTML to blocks (Simplified)
function parseHtmlToSlides(html: string): Slide[] {
  ...
  if (sections.length > 0) {
    sections.forEach(section => {
      ...
      section.childNodes.forEach(node => {
        if (node.nodeName === 'H1' || node.nodeName === 'H2') {
          blocks.push({ id: crypto.randomUUID(), type: 'heading', content: node.textContent || '' });
          ...
          (node as HTMLElement).querySelectorAll('li').forEach(li => {
            blocks.push({ id: crypto.randomUUID(), type: 'bullet', content: li.textContent || '' });
          });
        }
      });
    ...
    // Fallback: Create one slide
    slides.push({
      id: crypto.randomUUID(),
      blocks: [
        { id: crypto.randomUUID(), type: 'heading', content: 'Imported Content' },
        { id: crypto.randomUUID(), type: 'paragraph', content: doc.body.textContent?.slice(0, 200) || '' }
      ]
    });
  }
