 ...
 */
export async function generatePlan(input: PlanGeneratorInput): Promise<GenerationPlan> {
  const {
    prompt,
    title,
    selectedFormats,
  ...
  const formatDescriptions = selectedFormats.map(f => {
    const variant = selectedSubOptions[f] || 'default';
    return `${f} (${variant})`;
  }).join(', ');

  const supplementaryList = selectedSupplementary.length > 0
    ? `Additional outputs: ${selectedSupplementary.join(', ')}`
    : '';

  const modifiersList = selectedModifiers.length > 0
...
function createFallbackPlan(input: PlanGeneratorInput): GenerationPlan {
  const { prompt, selectedFormats, selectedSupplementary, selectedStyle, learnerLevel } = input;
  ...
  const outputs = selectedFormats.map(format => ({
    ...
    estimatedCount: format === 'slides' ? '~10-15 slides' : format === 'canvas' ? '1 canvas' : '~5-10 sections',
  }));
  ...
  if (selectedSupplementary.length > 0) {
    selectedSupplementary.forEach(supp => {
      outputs.push({
        format: supp,
        variant: 'standard',
        estimatedCount: supp === 'quiz' ? '5 questions' : supp === 'flashcards' ? '10-15 cards' : '1-2 pages',
      });
    });
  }
  ...
  return {
    ...
    outputs,
    structure: [
      { section: 'Introduction & Objectives', description: 'Learning objectives and overview', estimatedItems: 2 },
      { section: 'Core Content', description: 'Main educational material', estimatedItems: 5 },
      { section: 'Clinical Applications', description: 'Practical applications and examples', estimatedItems: 3 },
      { section: 'Summary & Key Points', description: 'Review and takeaways', estimatedItems: 2 },
    ],
    keyTopics,
    suggestedAdditions: ['Add clinical cases', 'Include recent guidelines', 'Add visual diagrams'],
 ...
 */
export function validatePlan(plan: GenerationPlan): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
