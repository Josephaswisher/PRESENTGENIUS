...
function postProcessFormattedHtml(html: string): string {
  ...
  processed = processed.replace(/<section(?!\s+id=)/g, (match, offset) => {
    const sectionsBefore = processed.substring(0, offset).match(/<section/g) || [];
    return `<section id="slide-${sectionsBefore.length + 1}"`;
  });
