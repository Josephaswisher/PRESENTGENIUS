/**
 * GLM (Zhipu AI) Provider - Direct API Access
 * Models: glm-4.7 (flagship), glm-4.6 (200K), glm-4.5, glm-4.5-air (lightweight)
 * Endpoint: https://api.z.ai/api/paas/v4/chat/completions
 * Latest: GLM-4.7 released Dec 2025 with enhanced coding & reasoning
 */
...
export class GLMProvider extends BaseProvider {
  constructor() {
    super({
      endpoint: 'https://api.z.ai/api/paas/v4/chat/completions',
      getApiKey: () => {
        // Support multiple environment variable names: VITE_GLM_API_KEY, GLM_API_KEY, ZAI_API_KEY
        const key = import.meta.env.VITE_GLM_API_KEY ||
                   import.meta.env.GLM_API_KEY ||
                   import.meta.env.ZAI_API_KEY;
        if (!key) {
          const error: any = new Error(
            'GLM API key is not configured. Please add it to your .env.local file and restart the app.'
          );
          error.validation = {
            isValid: false,
            suggestions: [
              'Create or edit .env.local in your project root',
              'Add this line: VITE_GLM_API_KEY=your-key-here',
              'Get your API key from https://z.ai/console/api-keys',
  ...
  /**
   * Generate new presentation using GLM API
   */
  async generate(
    prompt: string,
    files: FileInput[] = [],
    ...
    try {
      onProgress?.('starting', 0, '🔐 Validating GLM API key...');
      ...
      onProgress?.('glm', 8, '🟡 Initializing GLM (Zhipu AI) connection...');
      onProgress?.('glm', 10, '📋 Building system prompt...');
...
- Don't make it overly complex (balance wow factor with usability)`,
      ...
      onProgress?.('glm', 12, '✅ System prompt configured');
      ...
      if (files.length > 0) {
        ...
        for (const file of files) {
          if (file.mimeType.startsWith('image/')) {
            onProgress?.('glm', 17, `🖼️ Encoding image: ${file.mimeType}`);
            content.push({
              type: 'image_url',
              image_url: {
        ...
        onProgress?.('glm', 20, `✅ ${files.length} image(s) encoded`);
      } else {
        messages.push({ role: 'user', content: prompt });
        onProgress?.('glm', 18, '📝 User prompt added to messages');
      }

      onProgress?.('glm', 22, `📊 Request size: ${JSON.stringify(messages).length} characters`);
      onProgress?.('glm', 25, '🔗 Establishing connection to GLM API...');

      const requestFn = async () => {
        onProgress?.('glm', 30, '📡 Sending request to GLM...');
        onProgress?.('glm', 35, '⏳ Waiting for AI processing...');
        ...
        onProgress?.('glm', 60, '📥 Receiving response from GLM...');
        onProgress?.('glm', 65, '📦 Downloading response data...');
      ...
      this.handleProviderError(error, 'GLM');
    }
  }

  /**
   * Refine existing presentation using GLM API
   */
  async refine(
    currentHtml: string,
    instruction: string,
    ...
    try {
      ...
      // GLM uses more conservative limit (60KB instead of 80KB)
      ...
      this.logUsageWarnings(
        ...
        instruction.length,
        200000, // GLM 200K context limit
        modelId
      );

      if (wasTruncated) {
        console.warn(`⚠️ [GLM] Content truncated to prevent payload errors`);
        onProgress?.('processing', 12, '⚠️ Content truncated to fit size limits');
      } else {
        onProgress?.('processing', 12, '✅ Content fits within size limits');
      }

      onProgress?.('glm', 15, '📋 Building refinement prompt...');
...
Always prioritize clarity, medical accuracy, and educational effectiveness.`,
      ...
      onProgress?.('glm', 18, '✅ Prompt configured');
      onProgress?.('glm', 20, '🔗 Connecting to GLM API...');

      const requestFn = async () => {
        onProgress?.('glm', 25, '🎨 Sending refinement request...');
        onProgress?.('glm', 30, '⏳ AI is processing your request...');
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          console.error('❌ [GLM] Refinement Error:', {
            status: response.status,
            statusText: response.statusText,
            error: errorData,
            endpoint: this.config.endpoint,
          ...
          const error: any = new Error(
            errorData.error?.message ||
            errorData.message ||
            `GLM API error during refinement: ${response.status} ${response.statusText}`
          );
          error.status = response.status;
          error.statusCode = response.status;
          error.details = errorData;
        ...
        onProgress?.('glm', 60, '📥 Receiving refined content...');
        onProgress?.('glm', 65, '📦 Downloading response...');
        return response.json();
      };
