/**
 * Gemini Provider - Google AI Direct API Access
 * Models: gemini-3-flash-preview (1M context)
 * SDK: @google/genai@1.34.0
 * Features: Multimodal, thinking level, 3x faster than 2.5 Pro
 */
...
export class GeminiProvider extends BaseProvider {
      ...
      getApiKey: () => {
        ...
        if (!key) {
          const error: any = new Error(
            'Gemini API key is not configured. Please add it to your .env.local file and restart the app.'
          );
          error.validation = {
            isValid: false,
            suggestions: [
              'Create or edit .env.local in your project root',
              'Add this line: VITE_GEMINI_API_KEY=your-key-here',
  ...
  /**
   * Generate new presentation using Gemini 3 Flash
   */
  async generate(
    prompt: string,
    files: FileInput[] = [],
    ...
    try {
      onProgress?.('starting', 0, '🔐 Validating Gemini API key...');
      ...
      onProgress?.('gemini', 8, '🟡 Initializing Gemini 3 Flash connection...');
      onProgress?.('gemini', 10, '📋 Building system prompt...');

      // Build system instruction
      const systemInstruction = `You are an expert medical educator creating interactive HTML presentations for Dr. Swisher's medical education platform.
...
- Each slide should have a UNIQUE visual identity`;
      ...
      onProgress?.('gemini', 12, '✅ System prompt configured');
      ...
      if (files && files.length > 0) {
        onProgress?.('gemini', 15, `📷 Processing ${files.length} uploaded file(s)...`);

        for (const file of files) {
          // Gemini expects base64 data in inlineData format
          parts.push({
            inlineData: {
          ...
          onProgress?.('gemini', 18, `✅ Image processed (${file.mimeType})`);
        }
      }

      onProgress?.('gemini', 20, '🚀 Sending streaming request to Gemini 3 Flash...');
      ...
      await this.makeRequestWithRetry(async () => {
        ...
        for await (const chunk of stream) {
          if (chunk.text) {
            generatedHtml += chunk.text;

            // Update progress based on accumulated length
            const estimatedProgress = Math.min(20 + (generatedHtml.length / 200), 95);
            if (estimatedProgress > lastProgress + 5) {
              const kb = Math.round(generatedHtml.length / 1024);
              onProgress?.('gemini', estimatedProgress, `📥 Streaming... (${kb}KB received)`, undefined, generatedHtml);
      ...
      onProgress?.('gemini', 95, '✅ Response processed successfully');
      onProgress?.('gemini', 98, '📄 Extracting HTML from response...');

      // Extract HTML from response with multiple fallback patterns
      let htmlContent = '';

      // Try 1: Extract from markdown code blocks first
      const codeBlockMatch = generatedHtml.match(/```(?:html)?\s*(<!DOCTYPE[\s\S]*?<\/html>|<html[\s\S]*?<\/html>)\s*```/i);
      if (codeBlockMatch) {
        htmlContent = codeBlockMatch[1];
      } else {
        // Try 2: Standard HTML patterns
        const htmlMatch = generatedHtml.match(/<!DOCTYPE html>[\s\S]*?<\/html>/i) ||
                         generatedHtml.match(/<html[\s\S]*?<\/html>/i);
        if (htmlMatch) {
          htmlContent = htmlMatch[0];
        } else {
          // Try 3: Look for any HTML-like content as last resort
          const bodyMatch = generatedHtml.match(/<body[\s\S]*?<\/body>/i);
          if (bodyMatch) {
            // Wrap in minimal HTML structure
            htmlContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>${bodyMatch[0]}</html>`;
          } else {
            throw new Error('No valid HTML found in Gemini response. Response may be incomplete or malformed.');
          }
        }
      }
      ...
      try {
        const htmlWithCheckpoints = await injectCheckpointSlides(htmlContent, {
          interval: 3,
          provider: 'gemini', // Use Gemini for checkpoint questions (same provider)
        ...
        console.warn('⚠️ [Gemini] Failed to inject checkpoints, returning presentation without them:', checkpointError);
      ...
      throw new Error(
        error.message ||
        'Failed to generate presentation with Gemini. Please check your API key and try again.'
      );
    }
  }

  /**
   * Refine existing presentation using Gemini 3 Flash
   */
  async refine(
    currentHtml: string,
    instruction: string,
    ...
    try {
      ...
      onProgress?.('gemini', 10, '🟡 Preparing refinement request...');

      // Apply compression and truncation from BaseProvider
      const { html: truncatedHtml, wasTruncated } = this.truncateHtmlForRefinement(
        currentHtml,
        80000 // Conservative limit for Gemini (1M context available)
      );

      if (wasTruncated) {
        onProgress?.('gemini', 15, '⚠️ HTML truncated to fit context window');
      }
      ...
      onProgress?.('gemini', 20, '📋 Building refinement prompt...');
...
Return the refined HTML:`;
      ...
      onProgress?.('gemini', 25, '🚀 Sending streaming refinement request...');
      ...
      await this.makeRequestWithRetry(async () => {
        ...
        for await (const chunk of stream) {
          if (chunk.text) {
            refinedHtml += chunk.text;

            // Update progress
            const estimatedProgress = Math.min(25 + (refinedHtml.length / 200), 95);
            if (estimatedProgress > lastProgress + 5) {
              const kb = Math.round(refinedHtml.length / 1024);
              onProgress?.('gemini', estimatedProgress, `📥 Refining... (${kb}KB)`, undefined, refinedHtml);
              lastProgress = estimatedProgress;
            }
          }
        }
        ...
        if (!refinedHtml) {
          throw new Error('No text content in Gemini refinement response');
        }

        return refinedHtml;
      }, onProgress);

      onProgress?.('gemini', 95, '✅ Refinement complete');
      onProgress?.('gemini', 98, '📄 Extracting refined HTML...');

      // Extract HTML from response with multiple fallback patterns
      let extractedHtml = '';

      // Try 1: Extract from markdown code blocks first
      const codeBlockMatch = refinedHtml.match(/```(?:html)?\s*(<!DOCTYPE[\s\S]*?<\/html>|<html[\s\S]*?<\/html>)\s*```/i);
      if (codeBlockMatch) {
        extractedHtml = codeBlockMatch[1];
      } else {
        // Try 2: Standard HTML patterns
        const htmlMatch = refinedHtml.match(/<!DOCTYPE html>[\s\S]*?<\/html>/i) ||
                         refinedHtml.match(/<html[\s\S]*?<\/html>/i);
        if (htmlMatch) {
          extractedHtml = htmlMatch[0];
        } else {
          // Try 3: Look for any HTML-like content as last resort
          const bodyMatch = refinedHtml.match(/<body[\s\S]*?<\/body>/i);
          if (bodyMatch) {
            // Wrap in minimal HTML structure
            extractedHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>${bodyMatch[0]}</html>`;
          } else {
            throw new Error('No valid HTML found in Gemini refinement response. Response may be incomplete or malformed.');
          }
        }
      }
      ...
      throw new Error(
        error.message ||
        'Failed to refine presentation with Gemini. Please try again.'
      );
    }
  }
}
